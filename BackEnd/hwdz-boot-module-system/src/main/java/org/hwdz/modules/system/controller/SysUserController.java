package org.hwdz.modules.system.controller;
...
import org.hwdz.modules.system.entity.*;
import org.hwdz.modules.system.service.*;
import org.hwdz.modules.system.vo.SysDepartUsersVO;
...
import org.hwdz.modules.system.model.DepartIdModel;

import org.jeecg.common.system.query.QueryGenerator;
import org.jeecg.common.util.ImportExcelUtil;


import org.hwdz.modules.system.model.SysUserSysDepartModel;

import org.hwdz.modules.system.vo.SysUserRoleVO;
...
//		IPage<SysUser> pageList = sysUserService.page(page, queryWrapper);
        ...
        if(oConvertUtils.isNotEmpty(depId)){
            List<String> subDepids = sysDepartService.getSubDepIdsByDepId(depId);
            //查询所有部门下的userid
            if(subDepids != null && subDepids.size()>0) {
                LambdaQueryWrapper<SysUserDepart> sudWrapper = new LambdaQueryWrapper<>();
                sudWrapper.in(SysUserDepart::getDepId,subDepids);
                List<SysUserDepart> sysUserDeparts = sysUserDepartService.list(sudWrapper);
                List<String> depUserIds = sysUserDeparts.stream().map(SysUserDepart::getUserId).collect(Collectors.toList());
                if (CollectionUtils.isEmpty(depUserIds)){
                    return new Result<>();
                }
                queryWrapper.in(CollectionUtils.isNotEmpty(depUserIds),"id",depUserIds);
            }
        }
