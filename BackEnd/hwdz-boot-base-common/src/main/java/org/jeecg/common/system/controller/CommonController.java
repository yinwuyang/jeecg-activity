...
public class CommonController {
	...
	@ApiImplicitParams({
			@ApiImplicitParam(name = "biz",value = "自定义上传路径",paramType = "query",required = false),
			@ApiImplicitParam(name = "client",value = "上传文件客户端",paramType = "query",required = false),
	})
	...
	public Result<?> upload(HttpServletRequest request, HttpServletResponse response) {
		...
		if(oConvertUtils.isNotEmpty(savePath)){
			String orgName = savePath.substring(savePath.lastIndexOf("/")+1, savePath.length());
			//保存到oss表
			sysBaseAPI.saveOssFileInfo(orgName,savePath,client);
