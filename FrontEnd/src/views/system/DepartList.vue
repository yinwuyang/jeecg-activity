<template>
  <a-row class="mainContentHeight" :gutter="12">
    <a-col class="mainContentHeight" :md="12" :sm="24">
      <a-card class="TableCardClass" :bodyStyle="{ height: '100%' }" :bordered="false">
        <!-- 按钮操作区域 -->
        <a-row>
    ...
    <a-col class="mainContentHeight" :md="12" :sm="24">
      <a-tabs defaultActiveKey="1" style="height: 100%; background: #fff; padding: 0 0.15rem">
        <a-tab-pane tab="基本信息" key="1">
          <a-card :bordered="false" v-if="selectedKeys.length > 0">
            <a-form :form="form">
              ...
              <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="机构类型" v-if="dictItems.length != 0">
                ...
                <template v-else>
                  <a-radio-group v-decorator="['orgCategory', validatorRules.orgCategory]" placeholder="请选择机构类型">
                    <a-radio
                      v-for="item in dictItems.filter((el) => {
                        return el.value != '1'
                      })"
                      :key="item.value"
                      :value="item.value"
...
import { queryDepartTreeList, searchByKeywords, deleteByDepartId, ajaxGetDictItems } from '@/api/api'
...
export default {
  ...
  methods: {
    getDictItems() {
      ajaxGetDictItems('org_category').then((res) => {
        this.dictItems = res.result
      })
    },
    loadData() {
      this.getDictItems()
