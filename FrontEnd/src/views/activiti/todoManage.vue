...
<template>
  <div class="search">
        ...
        >
          ...
          <a-table-column title="操作" data-index align="center">
            <template slot-scope="t, r, i">
              ...
              >
                通过
                <a-divider type="vertical" />驳回 <a-divider type="vertical" />委托
                <a-divider type="vertical" />
              </span>
              <span v-else>
    ...
    <onl-cgform-auto-modal ref="modal" :code="code"></onl-cgform-auto-modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true
      postFormAction(this.url.todoList, this.searchForm).then((res) => {
        this.loading = false
        if (res.success) {
    ...
    //     this.$refs.modal.detail(formTemplate, r.tableId) //表单modle
    //   })
    // },
    passTask(v) {
