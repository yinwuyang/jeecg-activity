<template>
  <a-card :bordered="false">
    ...
    >
      ...
      <a-table-column title="标题" data-index="title" :width="150" align="center">
        <template slot-scope="t,r,i">
          <span>{{t}}</span>
        </template>
      </a-table-column>
      ...
      <a-table-column title="当前审批环节" data-index="currTaskName" :width="150" align="center">
        <template slot-scope="t,r,i">
          <span>{{t}}</span>
        </template>
      </a-table-column>
      ...
      <a-table-column title="操作" data-index align="center">
        <template slot-scope="t,r,i">
          <template v-if="r.status == 0">
            ...
            <a href="javascript:void(0);" @click="detail(r,true)" style="color: #000000">编辑</a>
            <a-divider type="vertical" />
    ...
    <onl-cgform-auto-modal v-if="code" ref="modal" :code="code"></onl-cgform-auto-modal>
  </a-card>
</template>
...
export default {
  ...
  methods: {
    ...
    getProcessList() {
      ...
      postFormAction(this.url.getProcessDataList, { status: 1, roles: true })
        .then(res => {
          this.activeKeyAll = []
          if (res.success) {
    ...
    applySubmit() {
      ...
      postFormAction(this.url.applyBusiness, params)
        .then(res => {
          if (res.success) {
    ...
    //   await getAction(`/online/cgform/api/getColumns/${r.routeName}`).then(res => {
    //     //异步请求这个参数~，不然第一次会报错点不开
    //     //表单数据
    //     this.formTemplate = res.result.formTemplate
    //   })
    //   if (isView) {
    //     this.$refs.modal.detail(this.formTemplate, r.tableId) //表单modle
    //   } else {
    //     this.$refs.modal.edit(this.formTemplate, r.tableId)
    //   }
    ...
    handelSubmitCancel() {
      this.submitLoading = true
      postFormAction(this.url.cancelApply, this.cancelForm)
        .then(res => {
          if (res.success) {
            this.$message.success('操作成功')
            this.loadData()
